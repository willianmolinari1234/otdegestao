...
/**
 * Quantos cupons cada PERFIL de loja deve manter.
 *
 * O 4 fixo para todas acusava loja certa. Loja de ticket baixo trabalha com
 * dois cupons só — o de 3% e o Prêmio de Seguidor — e cobrar quatro dela era
 * inventar falta, que é o mesmo erro do "3 descontos" por outro caminho.
 *
 * É perfil e não número solto de propósito: o número mora num lugar só. Se
 * amanhã o combinado do ticket baixo virar três cupons, muda aqui e vale para
 * todas as lojas daquele perfil, sem ninguém reeditar loja por loja.
 */
export const PERFIS_CUPOM = { padrao: 4, ticketbaixo: 2 };

/** Mínimo de cupons de um perfil. Perfil desconhecido ou vazio cai no padrão. */
export function minCuponsDoPerfil(perfil, minimos = MINIMO_FERRAMENTAS) {
  const p = String(perfil || "").trim().toLowerCase();
  return PERFIS_CUPOM[p] || minimos.cupom;
}
 ...
 *
 * @param {Array} lojas [{cliente, promocoes:[...], minCupons?}]  minCupons manda na loja
 * @param {number} agoraSeg momento de referência, em segundos
 * @param {{cupom:number,desconto:number}} minimos  usado quando a loja não traz o seu
 */
export function abaixoDoMinimo(lojas, agoraSeg, minimos = MINIMO_FERRAMENTAS, premio = PREMIO_SEGUIDOR) {
  ...
  for (const loja of lojas || []) {
    ...
    // O mínimo pode vir da PRÓPRIA loja (campo `minCupons`, posto por quem
    // monta a lista a partir do cadastro). Sem ele, vale o mínimo geral —
    // assim quem chamar sem saber de perfil nenhum continua funcionando.
    const minLoja = Number(loja.minCupons) > 0 ? Number(loja.minCupons) : minimos.cupom;

    const faltas = [];
    if (cupons.length < minLoja) faltas.push({ chave: "cupom", texto: `${cupons.length}/${minLoja} cupons` });
